...
{  
	...
	/// </summary>   
	/// <param name="go"></param>   
	public delegate void OnItemChange(GameObject go,bool Up);  
	...
	{  
		...
		m_scrollView.onDragFinished += ScrollViewDragFinished;
		m_scrollView.onMomentumMove += ScrollViewMoving;
		m_scrollView.onDragStarted += ScrollDragStart;
		// 设置Cull   
		m_scrollView.GetComponent<UIPanel>().cullWhileDragging = true;  
		
		m_grid = this.GetComponent<UIGrid>();  
		float _cellWidth = m_grid.cellWidth;  
		float _cellHeight = m_grid.cellHeight;  
		m_posParam = new Vector4(_cellWidth,  
		                         _cellHeight,  
		                         m_grid.arrangement == UIGrid.Arrangement.Horizontal ? 1 : 0,  
		                         m_grid.arrangement == UIGrid.Arrangement.Vertical ? 1 : 0);  
	}  

	void ScrollDragStart()
	{
		m_bDrag = true;
	}
	void ScrollViewDragFinished()
	{
		...
		Vector3 constraint = m_scrollView.panel.CalculateConstrainOffset(m_scrollView.bounds.min, m_scrollView.bounds.min);
		if (constraint.y <= 1f)
		{
...
//			NGUITools.SetActive(_item.gameObject, i < _count?true:false);  
		}  
//		m_grid.Reposition();
	}  
	...
	{  
		...
		OnItemChangeMsg(movedWidget.gameObject,lastVisiable);  
		m_itemList.RemoveAt(_sourceIndex);  
		Destroy(movedWidget.gameObject);
...
//		m_itemList.Insert(_targetIndex, movedWidget);  
	...
	void OnItemChangeMsg(GameObject go,bool Up)  
	{  
		if (m_pItemChangeCallBack != null)  
		{  
			m_pItemChangeCallBack(go,Up);  
		}  
	}  
