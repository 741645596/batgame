...
public class SummonPetAttribute : NDAttribute {
	...
	protected override int GetBaseAttrData(EffectType Type)
	{
		...
		{
			int AddStrength = Strength ;
			return ScriptM.Formula<int>("CALC_HP",m_info.m_hp,AddStrength);
		}
		else if(Type == EffectType.PhyAttack)
		{
			int AddStrength = Strength ;
			int AddAgility = Agility ;
			int AddIntelligence = Intelligence ;
			return ScriptM.Formula<int>("CALC_SOLDIER_PHYATTACK",m_phy_attack,m_main_proterty,AddStrength ,AddAgility,AddIntelligence);
		}
		else if(Type == EffectType.PhyDefense)
		{
			int AddStrength = Strength ;
			int AddAgility = Agility ;
			return ScriptM.Formula<int>("CALC_SOLDIER_PHYDEFEND",m_phy_defend,AddStrength,AddAgility);
		}
		else if(Type ==EffectType.MagicAttack)
		{
			int AddIntelligence = Intelligence ;
			return ScriptM.Formula<int>("CALC_SOLDIER_MAGICATTACK",m_magic_attack,AddIntelligence);
		}
		else if(Type ==EffectType.MagicDefense)
		{
			int AddIntelligence = Intelligence ;
			return ScriptM.Formula<int>("CALC_SOLDIER_MAGICDEFEND",m_magic_defend,AddIntelligence);
		}
		else if(Type ==EffectType.PhyCrit)
		{
			int AddAgility = Agility ;
			return ScriptM.Formula<int>("CALC_SOLDIER_PHYCRIT",m_phy_crit,AddAgility);
		}
		else if(Type ==EffectType.MagicCrit)
		{
			int AddIntelligence = Intelligence ;
			return ScriptM.Formula<int>("CALC_SOLDIER_MAGICCRIT",m_phy_crit,AddIntelligence);
		}
