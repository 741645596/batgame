...
public class   SummonPetGridRun : GridRun
{
    ...
    public SummonPetGridRun(Life role)
        : base(role)
    {
    ...
    public SummonPet RoleParent {
        get { return m_parent as SummonPet; }
    }
            ...
            if (WalkDIR == WalkDir.WALKRIGHT) {
                if (lfreeg != null) {
                    if (lfreeg.IsAttackStations()) {
                        ...
                        lfreeg.GetCampRoleList(m_parent.m_Core.m_Camp, ref samelist, ref unsamelist);
                ...
                if (rfreeg != null) {
                    if (rfreeg.IsAttackStations()) {
                        List<int> samelist = new List<int>();
                        List<int> unsamelist = new List<int>();
                        rfreeg.GetCampRoleList(m_parent.m_Core.m_Camp, ref samelist, ref unsamelist);
                        if (unsamelist.Count > 0) {
                            rfreeg = rfreeg.Right;
                        } else {
                            return rfreeg;
    ...
    {
        if (m_parent.m_Status.HaveState(StatusType.RunAway)) {
            m_Attack = false;
            return;
        }
        //没有路径
        ...
        if (First.state == RoleState.FALL) {
            ...
            m_NGrid =  MapPos;
            WalkDIR =  First.dir;
            /*if (!m_parent.m_Status.CheckStateBySkill(1022))
            {*/
    ...
    public void ChangeDeep(int deep)
    {
        ...
        Role r = m_parent as Role;
        r.m_ajustcount = 0;
        r.m_ajustfrompostion = r.m_thisT.localPosition;
        ...
        r.m_ajusttopostion = r.GetMapGrid().pos + offset;
    }
