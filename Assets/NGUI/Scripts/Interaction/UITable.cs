...
public class UITable : UIWidgetContainer
{
	...
	{
		List<Transform> ltable = children;
		if(ltable.Count == 0) return ;
		for(int i = 0; i < ltable.Count  ;  i ++ )
		{
			GameObject.DestroyImmediate(ltable[i].gameObject);
		}
		mChildren.Clear();
	}
    ...
    /// <summary>
    /// 是否是分割预置的判定（临时） 统一分割命名规范
    /// </summary>
    /// <param name="name"></param>
    /// <returns></returns>
    bool IsFenGe(string name)
    {
        return name == "PaoDanBingFenGeItem";
    }
	...
	protected void RepositionVariableSize (List<Transform> children)
	{
		...
		for (int i = 0, imax = children.Count; i < imax; ++i)
		{
            ...
            if (IsFenGe(t.name))
            {
                ...
                b.center = new Vector3(0, b.center.y, b.center.z);
            }
            ...
            if (++x >= columns && columns > 0  || IsFenGe(t.name))
			{
				x = 0;
				++y;
			}
		...
		for (int i = 0, imax = children.Count; i < imax; ++i)
		{
            ...
            if (++x >= columns && columns > 0 || IsFenGe(t.name))
			{
                ...
                if (i < children.Count - 1 && IsFenGe(children[i + 1].name) || IsFenGe(t.name))
                {
                    yOffset += 15.0f;
                }
			}
