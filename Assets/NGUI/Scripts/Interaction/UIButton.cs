...
{
    /// <summary>
    /// false 鎸夐挳骞朵笉浼氭敼鍙樻墍鍦╯prite琛ㄧ幇
    /// </summary>
    public bool IsTweenTarget = true;
	...
	/// 
	/// 
	public delegate void OnClickHandler (UIButton sender);
	public event OnClickHandler OnClickEventHandler;


    public delegate void OnPressDownHandler(UIButton sender);
    public event OnPressDownHandler OnPressDownEventHandler;

    public delegate void OnPressUpHandler(UIButton sender);
    public event OnPressUpHandler OnPressUpEventHandler;

	public delegate void OnDragHandler(UIButton sender);
	public event OnDragHandler OnDragEventHandler;
    ...
    protected override void OnPress(bool isPressed)
    {
        ...
        if (isPressed)
        {
            if (OnPressDownEventHandler!=null)
            {
                OnPressDownEventHandler(this);
            }
        }
        else
        {
            if (OnPressUpEventHandler!=null)
            {
                OnPressUpEventHandler(this);
            }
        }
    }
	...
	/// Set the hover state.
	/// </summary>
	/// <param name="isOver">鎮?仠鏃朵紶鍏?rue锛岀Щ鍑烘椂浼犲叆false</param>
	protected override void OnHover (bool isOver)
	{
		base.OnHover (isOver);
//
//		TweenScale btScale = this.tweenTarget.GetComponent<TweenScale>();
//		if(btScale && !btScale.enabled && isOver)
//		{
//			btScale.enabled = true;
//			btScale.ResetToBeginning();
//		}
	}
}
